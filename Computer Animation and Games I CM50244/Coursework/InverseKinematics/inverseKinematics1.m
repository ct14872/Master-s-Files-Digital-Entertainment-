function inverseKinematics1(p0,thetas,lens,rate,J_mode,interp_mode,sa,sd,ignore,goal_mode)
    ...
    while 1
        ...
        while penalty ~= 100 && (abs(dx)>0.001 || abs(dy) > 0.001)%~=0 && dy~=0%(abs(dx) > 0.002 || abs(dy) > 0.002)
            ...
            elseif J_mode == 3
                ...
                change_vec = [goal_position(1)-this_x; goal_position(2)-this_y];
                I = [1 0 0; 0 1 0; 0 0 1];
                change = J_pseudo_inv*(change_vec +J*z')-z';
                change = mod(change,360);
            end
